...
function summariseTrain(train, base, stations) {
    ...
    return {
        duration: train.duration, departureAt: absoluteTime(base, train.departure), arrivalAt: absoluteTime(base, train.arrival), transfers: train.transfers, stops: train.stops,
...
async function compareTimes(options, dependencies = {}) {
    ...
    const transferMinutes = wheelchair ? Number(options.transferMinutes ?? 8) : 4;
    if (!Number.isInteger(transferMinutes) || transferMinutes < 4 || transferMinutes > 30) throw inputError('Choose a transfer allowance from 4 to 30 minutes.');
    ...
    const [notices, facilities, traffic] = await Promise.all([
