...
const cacheDirectory = process.env.VERCEL ? path.join(os.tmpdir(), '.cache') : path.join(__dirname, '..', '.cache');
...
const crowdForecast = line => {
    const day = new Date(Date.now() + 8 * 3600000).toISOString().slice(0, 10);
    return cached(`crowd-forecast-${day}-${line}`, 1800000, async () => {
        const result = await request('PCDForecast', { TrainLine: line });
        if (!Array.isArray(result.value)) throw new Error('Invalid crowd forecast response');
        return result;
    });
};
const incidents = () => cached('traffic-incidents', 120000, () => allPages('TrafficIncidents'));
...
module.exports = { cached, request, alerts, arrivals, crowd, crowdForecast, incidents, facilities, busStops, busRoutes, cacheDirectory };
