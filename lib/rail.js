...
async function dataset() {
    ...
    refreshPromise = (async () => {
        ...
        parsed = { ...decode(buffer), loadedAt: Date.now(), updatedAt: stat?.mtime?.toISOString() || new Date().toISOString(), stale };
        return parsed;
...
async function getNetwork(now = new Date()) {
    ...
    return { data, network: { ...networkCache.network, seconds: singaporeDay(now).seconds } };
}
