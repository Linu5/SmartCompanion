...
const { crowdMatches } = require('./preferences');
...
function directCandidates(groups, origins, destinations, walkPace = 75) {
    ...
    for (const route of groups.values()) {
        for (let i = 0; i < route.length - 1; i++) {
            ...
            for (let j = i + 1; j < route.length; j++) {
                ...
                const walkMinutes = Math.ceil((board.walkMeters + alight.walkMeters) / walkPace);
                // Planning estimate, not an LTA bus journey-time field.
                const rideMinutes = Math.ceil(km / 18 * 60 + (j - i) * 0.35);
                candidates.push({ service: route[i].ServiceNo, direction: route[i].Direction, board, alight, stops: j - i, km: Math.round(km * 10) / 10, walkMinutes, rideMinutes, boardWalkMinutes: Math.ceil(board.walkMeters / walkPace), alightWalkMinutes: Math.ceil(alight.walkMeters / walkPace), originCode: route[0].BusStopCode, destinationCode: route.at(-1).BusStopCode, visit: route.slice(0, i + 1).filter(s => s.BusStopCode === board.BusStopCode).length, stopCodes: route.slice(i, j + 1).map(s => s.BusStopCode) });
            }
        }
    }
    return candidates;
}
function nextBoardable(service, candidate, now = Date.now(), wheelchair = false, crowding = 'any') {
    if (!service) return null;
    return ['NextBus', 'NextBus2', 'NextBus3'].map(key => service[key]).filter(bus => {
        if (wheelchair && !wheelchairBus(bus)) return false;
        if (!crowdMatches(bus, crowding)) return false;
...
async function alternatives(origin, destination, preference = 'fastest', exactStops = false, wheelchair = false, settings = {}) {
    const [stopResult, routeResult] = await Promise.all([lta.busStops(), lta.busRoutes()]);
    const stops = stopResult.data;
    const radius = settings.maxWalk ?? (preference === 'walking' ? 450 : 1000);
    const walkPace = settings.walkPace ?? 75, crowding = settings.busCrowding ?? 'any';
    ...
    const ranked = directCandidates(groups, origins, destinations, walkPace).sort((a, b) => {
    ...
    const options = unique.map(candidate => {
        ...
        const next = feed && !feed.stale ? nextBoardable(service, candidate, Date.now(), wheelchair, crowding) : null;
        const untilBus = next ? Math.max(0, Math.ceil((Date.parse(next.EstimatedArrival) - Date.now()) / 60000)) : null;
        return { ...candidate, type: 'bus', next, waitMinutes: untilBus, duration: untilBus === null ? null : untilBus + candidate.rideMinutes + candidate.alightWalkMinutes, available: !!next, updatedAt: feed?.updatedAt || null, stale: !!feed?.stale, basis: 'estimated', routeStops: candidate.stopCodes.map(c => stopMap.get(c)).filter(Boolean).map(s => ({ id: `stop:${s.BusStopCode}`, sourceId: s.BusStopCode, type: 'bus_stop', name: s.Description, lat: Number(s.Latitude), lon: Number(s.Longitude) })), coordinates: candidate.stopCodes.map(c => stopMap.get(c)).filter(Boolean).map(s => [s.Latitude, s.Longitude]) };
    }).filter(option => (!wheelchair && crowding === 'any') || !!option.next).sort((a, b) => {
    ...
    return { options, radius, walkPace, crowding, nearby: origins, updatedAt: routeResult.updatedAt, stale: routeResult.stale || stopResult.stale, note: `Direct buses only. Walking uses straight-line distance × 1.3 at ${walkPace} m/min; ride estimates use route distance at 18 km/h plus 21 seconds per stop. Check the pedestrian route and boarding stop.` };
}
